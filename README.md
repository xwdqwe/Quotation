...
- Cardsabi 实时品牌目录是报价页面和发送接口的唯一官方品牌来源；新增官方品牌无需修改代码即可选择和发送。
- 本地品牌与别名只用于识别微信群文本，解析后统一转换为实时 Cardsabi `categoryName`。卡速按 Cardsabi 官方品牌单独配置。
- 卡头（`bin`）：选择品牌后实时读取 Cardsabi 的该品牌 BIN 列表；发送前再次实时校验，有值时发送并参与同报价条件判断，空值不发送。
